...
function parseNr(input) {
  if (isNaN(+input)) {
    return numStrings[input];
  }
  return +input;
}

const instructions = getInput().split(/\r?\n/g);
let total = 0;
instructions.forEach((line) => {
  const regex = /[0-9]|one|two|three|four|five|six|seven|eight|nine/g;

  const firstNumb = line.match(regex)[0];
  let secondNumb = line.match(regex)[0];
  for (let i = 1; i < line.length; i++) {
    const match = line.substring(i).match(regex);
    if (!match) {
      break;
    }
    secondNumb = match[0];
  }
  const linetotal = `${parseNr(firstNumb)}${parseNr(secondNumb)}`;
  total += +linetotal;
});
