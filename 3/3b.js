...
function findGearPositions(line) {
...
function findNumbers(line) {
  ...
  let helper;
  const result = [];
  while ((helper = regex1.exec(line)) !== null) {
    const match = helper[0];
    const startIndex = regex1.lastIndex - match.length;
...
const numbersInLine = input.map((line) => findNumbers(line));

for (let i = 0; i < input.length; i++) {
  const line = input[i];
  const gearPositions = findGearPositions(line);
  gearPositions.forEach((gearPosition) => {
    const adjacentNumbersCurrentLine = numbersInLine[i]
      .filter((numberInLine) => {
        return (
          numberInLine.endIndex === gearPosition - 1 ||
          numberInLine.startIndex === gearPosition + 1
        );
      })
      .map((numberInLine) => numberInLine.value);
    const adjacentNumbersPreviousLine = numbersInLine[i - 1]
      ?.filter((numberInLine) => {
        return numberInLine.adjacentPositions.includes(gearPosition);
      })
      .map((numberInLine) => numberInLine.value);
    const adjacentNumbersNextLine = numbersInLine[i + 1]
      ?.filter((numberInLine) => {
        return numberInLine.adjacentPositions.includes(gearPosition);
      })
      .map((numberInLine) => numberInLine.value);
    const adjacentNumbers = [
      ...adjacentNumbersCurrentLine,
      ...adjacentNumbersPreviousLine,
      ...adjacentNumbersNextLine,
    ];
