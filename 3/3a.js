...
function findNumbers(line) {
  ...
  while ((helper = regex1.exec(line)) !== null) {
    ...
    result.push({
      startIndex: regex1.lastIndex - match.length,
      endIndex: regex1.lastIndex - 1,
      value: +match,
    });
  }
  return result;
...
for (let i = 0; i < input.length; i++) {
  ...
  numbersInLine?.forEach((numberInLine) => {
    const { startIndex, endIndex, value } = numberInLine;
    if (startIndex > 0 && line.charAt(startIndex - 1).match(/[^.0-9]/)) {
      sum += value;
    } else if (
      endIndex < line.length - 1 &&
      line.charAt(endIndex + 1).match(/[^.0-9]/)
    ) {
      sum += value;
    } else {
      const startPosition = startIndex === 0 ? startIndex : startIndex - 1;
      const endPosition =
        endIndex === line.length - 1 ? endIndex : endIndex + 1;

      for (let j = startPosition; j <= endPosition; j++) {
        if (
          input[i - 1]?.charAt(j)?.match(/[^.0-9]/) ||
          input[i + 1]?.charAt(j)?.match(/[^.0-9]/)
        ) {
          sum += value;
          break;
