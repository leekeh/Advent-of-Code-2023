const input = getInput().split(/\r?\n/g);

let possibleGamesTotalId = 0;

input.forEach((line) => {
  const id = +line.split(":")[0].substring("Game ".length);
  const sets = line.split(":")[1].split(";");
  let maxAmounts = { blue: 0, green: 0, red: 0 };
  sets.forEach((set) => {
    const colorPicks = set.split(",");
    colorPicks.forEach((colorPick) => {
      const [amount, color] = colorPick.trim().split(" ");
      const amountNr = +amount;
      if (amountNr > maxAmounts[color]) {
        maxAmounts[color] = amountNr;
      }
    });
  });
  if (maxAmounts.red <= 12 && maxAmounts.green <= 13 && maxAmounts.blue <= 14) {
    possibleGamesTotalId += id;
  }
});

console.log(possibleGamesTotalId);

function getInput() {
  return `Game 1: 1 blue; 4 green, 5 blue; 11 red, 3 blue, 11 green; 1 red, 10 green, 4 blue; 17 red, 12 green, 7 blue; 3 blue, 19 green, 15 red
